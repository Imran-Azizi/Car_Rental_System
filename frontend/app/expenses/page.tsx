...
function todayISO() {
  return formatKabulIso(new Date());
}
...
function getPeriodRange(period: ReportPeriod): {
  ...
  const todayKabul = formatKabulIso(now);
  const [year, month] = todayKabul.split("-");
  ...
  if (period === "daily") {
    return {
      dateFrom: todayKabul,
      dateTo: todayKabul,
      label: `امروز — ${formatAfghanDate(now.toISOString())}`,
    };
  }
  if (period === "weekly") {
    ...
    return {
      dateFrom: iso(start),
      dateTo: iso(now),
      label: `این هفته — ${formatAfghanDate(start.toISOString())} تا ${formatAfghanDate(now.toISOString())}`,
    };
  }
  if (period === "monthly") {
    return {
      dateFrom: monthStart,
      dateTo: todayKabul,
      label: `این ماه — ${formatAfghanDate(monthStart)} تا ${formatAfghanDate(now.toISOString())}`,
    };
  }
  if (period === "yearly") {
    return {
      dateFrom: yearStart,
      dateTo: todayKabul,
      label: `این سال — ${formatAfghanDate(yearStart)} تا ${formatAfghanDate(now.toISOString())}`,
    };
  }
...
export default function ExpensesPage() {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (receiptFile) {
        ...
        Object.entries(baseData).forEach(
  ...
  const SortIcon = ({ field }: { field: string }) => (
    <ArrowUpDown
      className={`w-3 h-3 inline ml-1 ${sortBy === field ? "text-amber-500" : "text-amber-300/50"}`}
    />
  );
  ...
  const handlePrint = async () => {
    ...
    const totalAdmin = reportExpenses.reduce(
      (s, e) => s + (e.adminShare || 0),
      0,
    );
    const totalOwner = reportExpenses.reduce(
      (s, e) => s + (e.ownerShare || 0),
      0,
    );
  ...
  const splitOwnerShare =
    Math.round((parsedModalAmount - splitAdminShare) * 100) / 100;
  ...
  const statCards = [
    {
      ...
      value: statsLoading
        ? "..."
        : `${formatNumber(stats?.today.amount ?? 0)} ${t.currency}`,
      icon: CalendarDays,
      color: "linear-gradient(135deg,#f59e0b,#d97706)",
      sub: statsLoading ? "" : `${stats?.today.count ?? 0} ردیف`,
    },
    {
      title: t.weekExpenses,
      value: statsLoading
        ? "..."
        : `${formatNumber(stats?.week.amount ?? 0)} ${t.currency}`,
      icon: BarChart3,
      color: "linear-gradient(135deg,#3b82f6,#2563eb)",
      sub: statsLoading ? "" : `${stats?.week.count ?? 0} ردیف`,
    },
    {
      title: t.monthExpenses,
      value: statsLoading
        ? "..."
        : `${formatNumber(stats?.month.amount ?? 0)} ${t.currency}`,
      icon: TrendingDown,
      color: "linear-gradient(135deg,#8b5cf6,#7c3aed)",
      sub: statsLoading ? "" : `${stats?.month.count ?? 0} ردیف`,
    },
    {
      title: t.totalExpenses,
      value: statsLoading
        ? "..."
        : `${formatNumber(stats?.total.amount ?? 0)} ${t.currency}`,
      icon: Wallet,
      color: "linear-gradient(135deg,#10b981,#059669)",
      sub: statsLoading ? "" : `${stats?.total.count ?? 0} ردیف`,
    },
  ];
            ...
            <div className="relative" ref={periodMenuRef}>
              <div className="flex items-center rounded-xl border border-amber-300 overflow-hidden">
                ...
                >
                  ...
                  <ChevronDown
                    className={`w-3.5 h-3.5 transition-transform ${showPeriodMenu ? "rotate-180" : ""}`}
                  />
                </button>
              </div>
            ...
            <table className="w-full table-golden min-w-[820px]">
              <thead>
                <tr>
                  ...
                  <th
                    className="px-4 py-3 text-right text-sm cursor-pointer hover:text-amber-700 select-none"
                    onClick={() => handleSort("fromWhom")}
                  >
                    {t.fromWhom}
                    <SortIcon field="fromWhom" />
                  </th>
                  <th
                    className="px-4 py-3 text-right text-sm cursor-pointer hover:text-amber-700 select-none"
                  ...
                  <th className="px-4 py-3 text-right text-sm w-28">
                    {t.actions}
                  </th>
                </tr>
              </thead>
                    ...
                    >
                      ...
                      <td className="px-4 py-3">
                        <div className="space-y-1">
                          ...
                          {exp.carId &&
                            (exp.adminShare > 0 || exp.ownerShare > 0) && (
          ...
          {pagination.totalPages > 1 && (
            <div className="flex items-center justify-between px-4 py-3 border-t border-amber-100">
              <p className="text-xs text-amber-600">
                {(page - 1) * 20 + 1}–{Math.min(page * 20, pagination.total)} از{" "}
                {pagination.total} ردیف
              </p>
              <div className="flex items-center gap-1">
                <button
                ...
                {[...Array(pagination.totalPages)].map((_, idx) => {
                  ...
                  if (
                    ...
                    if (p === 2 || p === pagination.totalPages - 1)
                      return (
                        <span key={p} className="px-1 text-amber-400 text-xs">
                          …
                        </span>
                      );
                    return null;
      ...
      >
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          ...
          <div>
            ...
            />
            {errors.fromWhom && (
              <p className="text-red-500 text-xs mt-1">{errors.fromWhom}</p>
            )}
          </div>
          ...
          <div>
            ...
            />
            {errors.toWhom && (
              <p className="text-red-500 text-xs mt-1">{errors.toWhom}</p>
            )}
          </div>
                ...
                <div className="grid grid-cols-3 gap-0 divide-x divide-x-reverse divide-emerald-100 bg-white">
                  <div className="p-4 text-center">
                    ...
                    <p className="text-lg font-black text-gray-800" dir="ltr">
                      {formatNumber(parsedModalAmount)}
                    </p>
                  ...
                  <div className="p-4 text-center bg-amber-50/60">
                    ...
                    <p className="text-lg font-black text-amber-900" dir="ltr">
                      {formatNumber(splitAdminShare)}
                    </p>
                  ...
                  <div className="p-4 text-center bg-teal-50/60">
                    ...
                    <p className="text-lg font-black text-teal-900" dir="ltr">
                      {formatNumber(splitOwnerShare)}
                    </p>
                    <p className="text-xs text-teal-500 mt-0.5">افغانی</p>
                  </div>
                </div>
                <div
                  className={`px-4 py-2.5 flex items-center gap-2 text-xs font-medium ${carHasOwner ? "bg-emerald-50 text-emerald-700" : "bg-amber-50 text-amber-700"}`}
                >
                  <span
                    className={`w-2 h-2 rounded-full shrink-0 ${carHasOwner ? "bg-emerald-500" : "bg-amber-400"}`}
                  />
                  {carHasOwner
      ...
      {detailExpense && (
        ...
        >
          <div className="space-y-5">
            ...
            >
              <div>
                ...
                <p className="text-3xl font-black text-amber-900" dir="ltr">
                  {formatNumber(detailExpense.amount)}{" "}
                  <span className="text-lg font-normal">افغانی</span>
                </p>
              </div>
              ...
              <table className="w-full text-sm">
                <tbody>
                  {[
                    {
                      label: "از طرف کی",
                      value: detailExpense.fromWhom,
                      icon: "👤",
                    },
                    { label: "به کی", value: detailExpense.toWhom, icon: "👤" },
                    {
                      label: "مقدار",
                      value: `${formatNumber(detailExpense.amount)} افغانی`,
                      icon: "💰",
                      ltr: true,
                    },
                    {
                      label: "تاریخ",
                      value: formatAfghanDate(detailExpense.date),
                      icon: "📅",
                    },
                    {
                      label: "موتر مرتبط",
                      value: detailExpense.car
                        ? `${detailExpense.car.carName} (${detailExpense.car.plateNumber})`
                        : "—",
                      icon: "🚗",
                    },
                    {
                      label: "توضیحات",
                      value: detailExpense.description || "—",
                      icon: "📝",
                    },
                    {
                      label: "ثبت توسط",
                      value: detailExpense.createdBy,
                      icon: "👔",
                    },
                  ].map((row, i) => (
                    <tr
                    ...
                    >
                      ...
                      <td
                        className={`px-4 py-3 text-amber-900 border-b border-amber-100 ${row.ltr ? "dir-ltr text-left" : ""}`}
                      >
                        {row.value}
                      </td>
                    </tr>
            ...
            {detailExpense.carId &&
              (detailExpense.adminShare > 0 ||
                detailExpense.ownerShare > 0) && (
                <div>
                  ...
                  <div className="grid grid-cols-2 gap-3">
                    <div className="flex items-center justify-between p-3 rounded-xl bg-amber-50 border border-amber-200">
                      ...
                      <span
                        ...
                        dir="ltr"
                      >
                        {formatNumber(detailExpense.adminShare)} ؋
                      </span>
                    </div>
                    <div className="flex items-center justify-between p-3 rounded-xl bg-teal-50 border border-teal-200">
                      ...
                      <span
                        ...
                        dir="ltr"
                      >
                        {formatNumber(detailExpense.ownerShare)} ؋
                      </span>
                    </div>
                  </div>
                </div>
            ...
            {detailExpense.receiptPhoto && (
              <div>
                ...
                <div
                  ...
                  onClick={() =>
                    setReceiptLightbox(imgUrl(detailExpense.receiptPhoto)!)
                  }
                >
                  <img
