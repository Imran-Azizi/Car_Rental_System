...
function canEditOrder(order: any): { allowed: boolean; reason?: string } {
  ...
  const elapsedHours =
    (Date.now() - new Date(order.completedAt).getTime()) / 3_600_000;
...
export default function AllOrdersPage() {
  ...
  const handleSaveTotalRent = async (orderId: string) => {
    ...
    if (isNaN(parsed) || parsed <= 0) {
      setTotalRentError(
        lang === "dari" ? "مقدار معتبر وارد کنید" : "سمه عدد ولیکئ",
      );
      return;
    }
    setTotalRentError("");
    ...
    try {
      ...
      setOrders((prev) =>
        prev.map((o) =>
          o.id === orderId
            ? {
                ...o,
                totalRent: updated.totalRent,
                remainingAmount: updated.remainingAmount,
                ownerShare: updated.ownerShare,
                adminShare: updated.adminShare,
      ...
      toast.success(
        lang === "dari" ? "مجموع کرایه بروز شد" : "د کرایې مجموع تازه شو",
      );
    } catch (err: any) {
  ...
  const orderImages = viewOrder
    ? [
        {
          url: imgUrl(viewOrder.customer?.photo),
          label: lang === "dari" ? "عکس مشتری" : "د مشتري انځور",
          filename: "customer-photo",
        },
        {
          url: imgUrl(viewOrder.guarantor?.photo),
          label: lang === "dari" ? "عکس ضامن (۱)" : "د ضامن انځور (۱)",
          filename: "guarantor-photo-1",
        },
        {
          url: imgUrl(viewOrder.guarantor?.photo2),
          label: lang === "dari" ? "عکس ضامن (۲)" : "د ضامن انځور (۲)",
          filename: "guarantor-photo-2",
        },
        {
          url: imgUrl(viewOrder.billDocPhoto),
          label: lang === "dari" ? "عکس بل / قرارداد" : "د بل انځور",
          filename: "bill-doc",
        },
        {
          url: imgUrl(viewOrder.tazkiraDocPhoto),
          label: lang === "dari" ? "عکس تذکره (۱)" : "د تذکرې انځور (۱)",
          filename: "tazkira-doc-1",
        },
        {
          url: imgUrl(viewOrder.tazkiraDocPhoto2),
          label: lang === "dari" ? "عکس تذکره (۲)" : "د تذکرې انځور (۲)",
          filename: "tazkira-doc-2",
        },
      ].filter(
        (img): img is { url: string; label: string; filename: string } =>
          !!img.url,
      )
    : [];
  ...
  const downloadImage = async (url: string, filename: string) => {
    try {
      ...
      toast.error(
        lang === "dari" ? "خطا در دانلود تصویر" : "د انځور ډاونلوډ کې تیروتنه",
      );
    }
  };
    ...
    <MainLayout>
              ...
              <tbody>
                {loading ? (
                  <tr>
                    <td
                      colSpan={9}
                      className="px-4 py-8 text-center text-amber-500"
                    >
                      {t.loading}
                    </td>
                  </tr>
                  ...
                  orders.map((c) => {
                    ...
                    return (
                      ...
                      >
                          ...
                          {isOverdue && overdueDays > 0 && (
                            <div className="flex items-center gap-1 mt-0.5">
                              ...
                              <span className="text-xs font-bold text-red-600">
                                +{overdueDays}{" "}
                                {lang === "dari" ? "روز تاخیر" : "ورځ ناوخته"}
                              </span>
                            </div>
                          )}
                        </td>
                              ...
                              <div className="flex items-center gap-1.5 group">
                                <div>
                                  <div className="text-sm font-medium">
                                    {formatCurrency(c.totalRent)}
                                  </div>
                                  {isOverdue && overdueCharge > 0 && (
                                    <div className="text-xs font-bold text-red-600">
                                      +{formatCurrency(overdueCharge)}{" "}
                                      {lang === "dari" ? "جریمه" : "جریمه"}
                                    </div>
                                  )}
                                </div>
                        ...
                        <td className="px-4 py-3">
                          <div
                            className={`text-sm font-medium ${c.remainingAmount > 0 ? "text-red-600" : "text-green-600"}`}
                          >
                        ...
                        <td className="px-4 py-3">
                          <div className="flex flex-col gap-1 items-end">
                            <Badge
                              variant={statusMap[c.status]?.variant}
                              label={statusMap[c.status]?.[lang] || c.status}
                            />
                            {isOverdue && overdueDays > 0 && (
                          ...
                          <div className="flex flex-wrap items-center gap-1">
                            ...
                            {(() => {
                              ...
                              return allowed ? (
                                <button
                                  onClick={() =>
                                    router.push(`/orders/new?edit=${c.id}`)
                                  }
          ...
          <div className="space-y-5">
            ...
            >
              <div className="flex items-center gap-3">
                ...
                <Badge
                  variant={statusMap[viewOrder.status]?.variant}
                  label={statusMap[viewOrder.status]?.[lang]}
                />
              </div>
              <div className="text-xs text-gray-500">
                {formatDate(viewOrder.createdAt || viewOrder.startDate)}
              </div>
            </div>
            ...
            <SectionTable
              ...
              rows={[
                [
                  lang === "dari" ? "نام موتر" : "د موتر نوم",
                  viewOrder.car?.carName,
                ],
                [
                  lang === "dari" ? "نمبر پلیت" : "بلیت نمبر",
                  viewOrder.car?.plateNumber,
                ],
            ...
            <SectionTable
              ...
              rows={[
                [
                  lang === "dari" ? "تاریخ تحویل" : "د ورلو نیټه",
                  `${formatDate(viewOrder.startDate)}${viewOrder.startTime ? "  " + viewOrder.startTime : ""}`,
                ],
                [
                  lang === "dari" ? "تاریخ برگشت" : "د راستون نیټه",
                  `${formatDate(viewOrder.endDate)}${viewOrder.endTime ? "  " + viewOrder.endTime : ""}`,
                ],
                [
                  lang === "dari" ? "کرایه روزانه" : "ورځنۍ کرایه",
                  formatCurrency(
                    viewOrder.rentPrice || viewOrder.dailyRate || 0,
                  ),
                  false,
                  "text-amber-800 font-bold",
                ],
                [
                  lang === "dari" ? "نرخ جریمه تأخیر" : "د ځنډ جریمه نرخ",
                  viewOrder.liveDelayPenaltyRate ||
                  viewOrder.delayPenaltyRate ||
                  viewOrder.rentPrice
                    ? formatCurrency(
                        viewOrder.liveDelayPenaltyRate ||
                          viewOrder.delayPenaltyRate ||
                ...
                [
                  ...
                  formatCurrency(viewOrder.totalRent),
                  false,
                  "text-amber-900 font-black text-base",
                ],
              ]}
            />
            ...
            <SectionTable
              ...
              rows={[
                [
                  lang === "dari" ? "پیش پرداخت" : "مخکنۍ تادیه",
                  formatCurrency(viewOrder.advancePayment),
                  false,
                  "text-green-700 font-bold",
                ],
                [
                  lang === "dari" ? "باقی مانده" : "پاتې مبلغ",
                  formatCurrency(viewOrder.remainingAmount),
                  false,
                  viewOrder.remainingAmount > 0
                    ? "text-red-600 font-bold"
                    : "text-green-700 font-bold",
                ],
                [
                  lang === "dari" ? "سهم صاحب موتر" : "د موتر د خاوند برخه",
                  formatCurrency(viewOrder.ownerShare || 0),
                ],
                [
                  lang === "dari" ? "سهم ادمین" : "د ادمین برخه",
                  formatCurrency(viewOrder.adminShare || 0),
                ],
              ]}
            />
            ...
            {(viewOrder.status === "OVERDUE" ||
              viewOrder.liveOverdueCharges > 0 ||
              viewOrder.overdueCharges > 0) && (
                ...
                <div className="p-4 bg-red-50 space-y-3">
                    ...
                    {[
                      {
                        ...
                        value: formatCurrency(
                          viewOrder.totalRent -
                            (viewOrder.liveTotalDelayPenalty ??
                              viewOrder.totalDelayPenalty ??
                              viewOrder.overdueCharges ??
                              0),
                        ),
                        cls: "text-gray-800",
                      ...
                      {
                        label:
                          lang === "dari" ? "روزهای تأخیر" : "د ناوخته ورځې",
                  ...
                  <div className="flex items-center justify-between px-4 py-3 rounded-xl border-2 border-red-400 bg-red-100">
                    <span className="text-sm font-bold text-red-800">
                      {lang === "dari"
                        ? "مبلغ قابل پرداخت نهایی (اصلی + جریمه)"
                        : "وروستی قابل پرداخت مبلغ (اصلي + جریمه)"}
                    </span>
            ...
            <SectionTable
              ...
              rows={
                [
                  ...
                  [
                    lang === "dari" ? "نام پدر" : "د پلار نوم",
                    viewOrder.customer?.fatherName,
                  ],
                  [
                    lang === "dari" ? "نام پدرکلان" : "د نیکه نوم",
                    viewOrder.customer?.grandfatherName,
                  ],
                  [
                    lang === "dari" ? "نمبر تذکره" : "د تذکرې نمبر",
                    viewOrder.customer?.tazkiraNumber,
                  ],
                  [
                    lang === "dari" ? "شماره تلفن" : "تلیفون شمیره",
                    viewOrder.customer?.phoneNumber,
                    true,
                  ],
                  [
                    lang === "dari" ? "شغل" : "مسلک",
                    viewOrder.customer?.occupation,
                  ],
                  [
                    lang === "dari" ? "ولایت" : "ولایت",
                    viewOrder.customer?.province,
                  ],
                  [
                    lang === "dari" ? "ناحیه" : "ناحیه",
                    viewOrder.customer?.district,
                  ],
                  [
                    lang === "dari" ? "قریه" : "ولسوالۍ",
                    viewOrder.customer?.village,
                  ],
                  [
                    lang === "dari" ? "آدرس فعلی" : "اوسنی پته",
                    viewOrder.customer?.currentAddress,
                  ],
                  [
                    lang === "dari" ? "آدرس دایمی" : "دایمي پته",
                    viewOrder.customer?.permanentAddress,
                  ],
              ...
              <SectionTable
                ...
                rows={
                  [
                    [
                      lang === "dari" ? "نام و تخلص" : "نوم او تخلص",
                      viewOrder.guarantor.fullName,
                    ],
                    [
                      lang === "dari" ? "نام پدر" : "د پلار نوم",
                      viewOrder.guarantor.fatherName,
                    ],
                    [
                      lang === "dari" ? "نام پدرکلان" : "د نیکه نوم",
                      viewOrder.guarantor.grandfatherName,
                    ],
                    [
                      lang === "dari" ? "نمبر تذکره" : "د تذکرې نمبر",
                      viewOrder.guarantor.tazkiraNumber,
                    ],
                    [
                      lang === "dari" ? "شماره تلفن" : "تلیفون شمیره",
                      viewOrder.guarantor.phoneNumber,
                      true,
                    ],
                    [
                      lang === "dari" ? "رابطه" : "اړیکه",
                      viewOrder.guarantor.relationship,
                    ],
                    [
                      lang === "dari" ? "ولایت" : "ولایت",
                      viewOrder.guarantor.province,
                    ],
                    [
                      lang === "dari" ? "ناحیه" : "ناحیه",
                      viewOrder.guarantor.district,
                    ],
                    [
                      lang === "dari" ? "آدرس فعلی" : "اوسنی پته",
                      viewOrder.guarantor.currentAddress,
                    ],
                  ].filter((r) => r[1]) as [string, any, boolean?, string?][]
                }
              />
            ...
            {(viewOrder.driverName ||
              viewOrder.driverLicense ||
              ...
              <SectionTable
                ...
                rows={
                  [
                    [
                      lang === "dari" ? "نام راننده" : "د دریور نوم",
                      viewOrder.driverName,
                    ],
                    [
                      lang === "dari" ? "نمبر لیسنس" : "لیسنس نمبر",
                      viewOrder.driverLicense,
                      true,
                    ],
                    [
                      lang === "dari" ? "شماره تلفن" : "تلیفون",
                      viewOrder.driverPhone,
                      true,
                    ],
                  ].filter((r) => r[1]) as [string, any, boolean?, string?][]
                ...
                <div className="rounded-2xl border border-green-200 overflow-hidden">
                  <table className="w-full text-sm">
                    <thead>
                      ...
                      >
                        <th className="px-4 py-2.5 text-right text-white text-xs font-semibold">
                          #
                        </th>
                        <th className="px-4 py-2.5 text-right text-white text-xs font-semibold">
                          {lang === "dari" ? "مبلغ" : "مبلغ"}
                        ...
                        <th className="px-4 py-2.5 text-right text-white text-xs font-semibold">
                          {lang === "dari" ? "تاریخ" : "نیټه"}
                        </th>
                        <th className="px-4 py-2.5 text-right text-white text-xs font-semibold">
                          {lang === "dari" ? "یادداشت" : "نوټ"}
                        </th>
                      </tr>
                    </thead>
                    <tbody>
                      {viewOrder.payments.map((p: any, i: number) => (
                        <tr
                          key={p.id}
                          className={
                            i % 2 === 0 ? "bg-white" : "bg-green-50/50"
                          }
                        >
                          <td className="px-4 py-2.5 text-green-600 font-medium text-xs border-b border-green-100">
                            {i + 1}
                          </td>
                          <td className="px-4 py-2.5 text-green-800 font-bold border-b border-green-100">
                            {formatCurrency(p.amount)}
                          </td>
                          <td className="px-4 py-2.5 text-green-700 text-xs border-b border-green-100">
                            {formatAfghanDate(p.paymentDate)}
                          </td>
                          <td className="px-4 py-2.5 text-green-600 text-xs border-b border-green-100">
                            {p.notes || "—"}
                          </td>
                        </tr>
                      ))}
              ...
              {orderImages.length === 0 ? (
                <div className="flex flex-col items-center justify-center py-8 rounded-xl border-2 border-dashed border-amber-200 bg-amber-50/50">
                  ...
                  <p className="text-sm font-semibold text-amber-500">
                    {lang === "dari"
                      ? "هیچ تصویری برای این سفارش ثبت نشده است"
                      : "د دې سفارش لپاره هیڅ انځور نشته"}
                  </p>
                </div>
              ) : (
                <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
                  {orderImages.map((img, idx) => (
                    <div
                      key={idx}
                      className="group rounded-xl overflow-hidden border-2 border-amber-100 bg-white shadow-sm hover:shadow-md transition-all"
                    >
                      <div
                      ...
                      <div className="p-2.5 border-t border-amber-100">
                        <p className="text-xs font-semibold text-amber-800 truncate text-center mb-1.5">
                          {img.label}
                        </p>
                        <button
                          onClick={() =>
                            downloadImage(
                              img.url,
                              `${viewOrder.contractNumber}-${img.filename}`,
                            )
                          }
              ...
              {(viewOrder.status === "ACTIVE" ||
                ...
                <div className="flex gap-2 flex-wrap justify-end">
                  ...
                  <button
                    onClick={() => {
                      const totalOwed =
                        (viewOrder.liveFinalTotal ?? viewOrder.totalRent) -
                        viewOrder.advancePayment;
                      if (totalOwed > 0) {
      ...
      <Modal
        ...
        title={
          pendingReturnId
            ? lang === "dari"
              ? "تسویه باقی و برگشت موتر"
              : "پاتې تادیه او راستون"
            : t.addPayment
        }
        size="sm"
      >
            ...
            <div className="p-3 bg-amber-50 rounded-lg text-sm">
              <p className="text-amber-700">
                ...
                <span className="font-bold text-red-600">
                  {formatCurrency(paymentModal.remainingAmount)}
                </span>
              </p>
            </div>
          )}
      ...
      <ConfirmDialog
        ...
        message={
          lang === "dari"
            ? "آیا موتر برگشت داده شده است؟"
            : "ایا موټر راستون شوی دی؟"
        }
      />
      <ConfirmDialog
        open={!!deleteId}
          ...
          >
            ...
            <div className="flex items-center justify-between mb-3 gap-3">
              <div className="flex items-center gap-3">
                <span className="text-white/50 text-sm font-mono">
                  {lightboxIdx + 1} / {orderImages.length}
                </span>
                <p className="text-white font-semibold text-sm">
                  {orderImages[lightboxIdx].label}
                </p>
              </div>
              <div className="flex items-center gap-2">
                <button
            ...
            {orderImages.length > 1 && (
              <div className="flex items-center justify-center gap-4 mt-4">
                ...
                <button
                  onClick={() =>
                    setLightboxIdx((lightboxIdx + 1) % orderImages.length)
                  }
                  className="w-10 h-10 rounded-full bg-white/10 hover:bg-white/25 flex items-center justify-center text-white transition-colors"
                >
