...
import {
  ...
  numericInputHandler,
  formatNumber,
  gregorianToAfghanParts,
  shamsiToGregorianIso,
  getAfghanDaysInMonth,
  toEnglishNums,
} from "@/lib/utils";
import {
  Car,
...
/* ─── style helpers ─── */
const inp =
  "w-full px-3 py-2.5 rounded-lg border-2 border-amber-200 bg-white text-sm focus:border-amber-500 focus:ring-2 focus:ring-amber-200 outline-none transition-all";
const eInp =
  "w-full px-3 py-2.5 rounded-lg border-2 border-red-400 bg-red-50 text-sm focus:border-red-500 focus:ring-2 focus:ring-red-100 outline-none transition-all";
...
function AfghanDatePicker({
  ...
  const defaultParts = useMemo(() => gregorianToAfghanParts(new Date()), []);
  const currentYear = useMemo(
    () => Number(defaultParts.year) || new Date().getFullYear() - 621,
    [defaultParts.year],
  );
  const yearMin = currentYear - 8;
  const yearMax = currentYear + 2;

  const daysInMonth = useMemo(
    () => getAfghanDaysInMonth(year || String(currentYear), month || "1"),
  ...
  useEffect(() => {
    if (value) {
      const parts = gregorianToAfghanParts(value);
      setYear(parts.year);
    ...
    if (defaultToday && !autoFilledRef.current) {
      autoFilledRef.current = true;
      if (defaultParts.year && defaultParts.month && defaultParts.day) {
        setYear(defaultParts.year);
        setMonth(defaultParts.month);
        setDay(defaultParts.day);
        const iso = shamsiToGregorianIso(
          defaultParts.year,
          defaultParts.month,
          defaultParts.day,
        );
  ...
  }, [value, defaultToday, defaultParts, onChange]);
  ...
  const normalizeNumberInput = (value: string) =>
    toEnglishNums(value).replace(/[^0-9]/g, "");
  ...
  const handleChange = (
    ...
    const normalizedDay = nextDay && nextMonth && nextYear ? nextDay : "";
    setYear(nextYear);
    setMonth(nextMonth);
    setDay(normalizedDay);

    if (nextYear && nextMonth && normalizedDay) {
      const iso = shamsiToGregorianIso(nextYear, nextMonth, normalizedDay);
      onChange(iso);
    } else {
      onChange("");
    }
  ...
  const shamsiLabel = year && month && day ? `${day}/${month}/${year}` : "";

  const handleCopy = async () => {
    if (!shamsiLabel) return;
    await navigator.clipboard.writeText(shamsiLabel);
      ...
      <div className="grid grid-cols-3 gap-3">
        ...
        <div className="space-y-1">
          ...
          <input
            ...
            value={year}
            onChange={(e) =>
              handleChange(normalizeNumberInput(e.target.value), month, "")
            }
            className={`${inp} bg-slate-50 appearance-none`}
            placeholder="Year"
          />
        </div>
      </div>
...
function MultiDocSlot({
  ...
  const handleChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (oversized.length) {
      toast.error("یک یا چند فایل بیشتر از 5MB است");
      return;
    }
  ...
  return (
    <div className="flex flex-col gap-3">
      ...
      <div
        className="rounded-xl border-2 border-dashed p-3 space-y-3"
        style={{
          borderColor: filledCount > 0 ? "#6ee7b7" : `${color}60`,
          background: filledCount > 0 ? "#f0fdf4" : `${color}08`,
        }}
      >
        {/* Uploaded thumbnails */}
        {filledCount > 0 && (
        ...
        {filledCount < maxFiles && (
          ...
          >
            ...
            <p className="text-xs text-amber-400">
              JPG, PNG, WEBP • حداکثر 5MB هر فایل
            </p>
          </button>
        )}
...
export default function OrderNewPage() {
  ...
  const [guarantorPhotoPreview, setGuarantorPhotoPreview] = useState<
    string | null
  >(null);
  const [guarantorPhoto2, setGuarantorPhoto2] = useState<File | null>(null);
  const [guarantorPhoto2Preview, setGuarantorPhoto2Preview] = useState<
    string | null
  >(null);
  const [billDocPhoto, setBillDocPhoto] = useState<File | null>(null);
  const [billDocPhotoPreview, setBillDocPhotoPreview] = useState<string | null>(
    null,
  );
  const [tazkiraDocPhoto, setTazkiraDocPhoto] = useState<File | null>(null);
  const [tazkiraDocPhotoPreview, setTazkiraDocPhotoPreview] = useState<
    string | null
  >(null);
  const [tazkiraDocPhoto2, setTazkiraDocPhoto2] = useState<File | null>(null);
  const [tazkiraDocPhoto2Preview, setTazkiraDocPhoto2Preview] = useState<
    string | null
  >(null);
  ...
  const [existingGuarantorId, setExistingGuarantorId] = useState<string | null>(
    null,
  );
  ...
  const saveDraft = async (silent = true) => {
    ...
    try {
      ...
      if (!silent)
        toast.error(
          lang === "dari"
            ? "خطا در ذخیره پیش‌نویس"
            : "د مسودې ذخیره کې تیروتنه",
        );
    } finally {
  ...
  useEffect(() => {
    ...
    ordersAPI
      ...
      .then((res) => {
        ...
        const cp = resolveImgUrl(c.customer?.photo);
        if (cp) setCustomerPhotoPreview(cp);
        const gp = resolveImgUrl(c.guarantor?.photo);
        if (gp) setGuarantorPhotoPreview(gp);
        const gp2 = resolveImgUrl(c.guarantor?.photo2);
        if (gp2) setGuarantorPhoto2Preview(gp2);
        const bp = resolveImgUrl(c.billDocPhoto);
        if (bp) setBillDocPhotoPreview(bp);
        const tp = resolveImgUrl(c.tazkiraDocPhoto);
        if (tp) setTazkiraDocPhotoPreview(tp);
        const tp2 = resolveImgUrl(c.tazkiraDocPhoto2);
        if (tp2) setTazkiraDocPhoto2Preview(tp2);
      })
      .catch(() =>
        toast.error(
          lang === "dari"
            ? "خطا در بارگذاری سفارش"
            : "د سفارش بارولو کې تیروتنه",
        ),
      );
  }, [token]);
  ...
  useEffect(() => {
    if (skipNextDailyRateSync.current) {
      skipNextDailyRateSync.current = false;
      return;
    }
  ...
  const rentalDays = useMemo(() => {
    ...
    const d = Math.ceil(
      (new Date(customer.endDate).getTime() -
  ...
  const dailyRent = useMemo(
    () => parseNum(dailyRentInput) || 0,
    [dailyRentInput],
  );
  const totalRent = useMemo(
    () => dailyRent * rentalDays,
    [dailyRent, rentalDays],
  );
  const received = useMemo(
    () => Math.min(parseNum(receivedAmount) || 0, totalRent),
    [receivedAmount, totalRent],
  );
  const remaining = useMemo(
    () => Math.max(0, totalRent - received),
    [totalRent, received],
  );
  const ownerShare = useMemo(
    () => Math.round(totalRent * 0.5 * 100) / 100,
    [totalRent],
  );
  const adminShare = useMemo(
    () => Math.round((totalRent - ownerShare) * 100) / 100,
    [totalRent, ownerShare],
  );
  ...
  const validateStep0 = () => {
    if (!selectedCarId) {
      toast.error("لطفاً یک موتر انتخاب کنید");
      return false;
    }
    return true;
  };
  const validateStep1 = (): boolean => {
    ...
    if (
      customer.startDate &&
      customer.endDate &&
      customer.endDate <= customer.startDate
    )
  ...
  const validateStep3 = (): boolean => {
    ...
    if (dailyRent <= 0)
      e.dailyRent =
        lang === "dari"
          ? "کرایه روزانه الزامی است و باید بیشتر از صفر باشد"
          : "ورځنۍ کرایه اړینه ده او باید له صفر زیات وي";
    if (rentalDays === 0)
      e.dates =
        lang === "dari"
          ? "تاریخ‌های کرایه را در مرحله قبل تنظیم کنید"
          : "د کرایې نیټې د مخکنۍ مرحلې کې تنظیم کړئ";
    setBillingErrors(e);
    return Object.keys(e).length === 0;
  };
    ...
    try {
      ...
      if (guarantor.fullName.trim()) {
        const guarData = {
          ...
          currentAddress: guarantor.currentAddress,
      ...
      if (hasDocs) {
        const fd = new FormData();
        Object.entries(contractBase).forEach(([k, v]) =>
      ...
      const customerBillData: CustomerBillData = {
        ...
        carType: [selectedCar?.carName, selectedCar?.model]
          .filter(Boolean)
          .join(" — "),
        plateNumber: selectedCar?.plateNumber,
      ...
      localStorage.setItem(
        "lastCustomerBill",
        JSON.stringify(customerBillData),
      );

      // Delete draft from server after successful submission
      if (draftId) {
        draftsAPI.delete(draftId).catch(() => {});
      }
      toast.success(
        lang === "dari"
          ? "سفارش موفقانه ثبت شد!"
          : "سفارش بریالیتوب سره ثبت شو!",
      );
  ...
  const handleEditSubmit = async () => {
    ...
    try {
      ...
      let finalGuarantorId: string | undefined =
        existingGuarantorId || undefined;
      if (guarantor.fullName.trim()) {
      ...
      toast.success(
        lang === "dari"
          ? "سفارش موفقانه ویرایش شد!"
          : "سفارش بریالیتوب سره سم شو!",
      );
      router.push("/orders");
    } catch (err: any) {
      toast.error(err.response?.data?.message || t.error);
  ...
  const steps = [
    ...
    {
      label: lang === "dari" ? "معلومات مشتری" : "د مشتري معلومات",
      icon: User,
    },
    {
      label: lang === "dari" ? "معلومات ضامن" : "د ضامن معلومات",
      icon: Shield,
    },
    {
      label: lang === "dari" ? "معلومات راننده" : "د دریور معلومات",
      icon: UserCheck,
    },
    { label: lang === "dari" ? "محاسبه مالی" : "مالي حساب", icon: Receipt },
    {
      label: lang === "dari" ? "اسناد و ثبت" : "اسناد او ثبت",
      icon: FileImage,
    },
  ];

  const fmtCur = (n: number) =>
    `${formatNumber(n)} ${lang === "dari" ? "افغانی" : "افغاني"}`;
        ...
        {showDraftBanner && !isEditMode && (
          <div className="flex items-center justify-between p-3 rounded-xl bg-blue-50 border border-blue-200 text-sm">
            <div className="flex items-center gap-2 text-blue-800">
              ...
              {lang === "dari"
                ? "پیش‌نویس بازیابی شد. اطلاعات قبلی بارگذاری شد."
                : "مسوده بیرته راغله. پخوانۍ معلومات بارګزاري شول."}
            </div>
            <button
          ...
          {steps.map((s, i) => {
            ...
            return (
              <div key={i} className="flex items-center flex-1">
                ...
                {i < steps.length - 1 && (
                  <div
                    className={`h-1 flex-1 mx-0.5 rounded-full ${i < step ? "bg-green-400" : "bg-amber-100"}`}
                  />
                )}
              </div>
            );
        ...
        >
            ...
            <div className="space-y-5">
              ...
              <div className="flex flex-col sm:flex-row gap-3">
                <div className="relative flex-1">
                  ...
                  <input
                    ...
                    placeholder={
                      lang === "dari" ? "جستجوی موتر..." : "موتر لټول..."
                    }
                    className={`${inp} pr-9`}
                  />
                  {search && (
                    <button
                      onClick={() => setSearch("")}
                      className="absolute left-3 top-1/2 -translate-y-1/2 text-amber-400 hover:text-red-500"
                    >
                      <X className="w-3.5 h-3.5" />
                    </button>
                  )}
                ...
                <div className="flex gap-2 shrink-0 flex-wrap">
                  {[
                    ...
                    {
                      key: "AVAILABLE",
                      label: lang === "dari" ? "موجود" : "موجود",
                    },
                    {
                      key: "RENTED",
                      label: lang === "dari" ? "کرایه شده" : "کرایه",
                    },
                    {
                      key: "MAINTENANCE",
                      label: lang === "dari" ? "تعمیر" : "ترمیم",
                    },
                  ].map((f) => (
                    <button
                      key={f.key}
              ...
              {carsLoading ? (
                ...
                <div className="text-center py-12 text-amber-400">
                  ...
                  <p>
                    {lang === "dari" ? "موتری یافت نشد" : "موتر ونه موندل شو"}
                  </p>
                </div>
              ) : (
                  ...
                  {filteredCars.map((car) => {
                    ...
                    return (
                      ...
                      >
                        {sel && (
                          <span
                            className="absolute top-2.5 left-2.5 z-10 w-7 h-7 rounded-full flex items-center justify-center text-white shadow-lg"
                            style={{
                              background:
                        ...
                        <div className="p-3.5 space-y-2.5">
                          ...
                          <div className="flex items-center justify-between gap-2">
                            <h4 className="font-bold text-amber-900 text-sm truncate flex-1">
                              {car.carName}
                            </h4>
                            <span
                              className={`text-[10px] px-2 py-0.5 rounded-full border font-medium shrink-0 ${si.cls}`}
                            >
                              {si.text}
                            </span>
                          </div>
                          ...
                          {/* Plate + rate row */}
                          <div className="flex items-center justify-between">
                            <div className="text-xs text-amber-600 font-mono bg-amber-50 px-2 py-1 rounded truncate max-w-[140px]">
                              {car.plateNumber}
                            </div>
                          </div>
                        </div>
              ...
              {selectedCar &&
                (() => {
                  const selThumb = selectedCar.images?.[0]?.url
                    ? resolveImgUrl(selectedCar.images[0].url)
                    : null;
                  return (
                    <div className="flex items-center gap-4 p-4 rounded-xl bg-green-50 border-2 border-green-200">
                      ...
                      {selThumb && (
                        <img
                          ...
                          className="w-14 h-14 rounded-lg object-cover border-2 border-green-300 shrink-0"
                        />
                      )}
                      <div className="flex-1 min-w-0">
                        <div className="flex items-center gap-2 flex-wrap">
                          ...
                          {selectedCar.carType && (
                            <span className="text-[10px] font-semibold bg-green-200 text-green-800 px-2 py-0.5 rounded-full">
                              {selectedCar.carType}
                            </span>
                          )}
                        </div>
                        <p className="text-sm text-green-600 truncate">
                          {selectedCar.model} — {selectedCar.color} —{" "}
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                ...
                >
                  <input
                    ...
                    className={customerErrors.fullName ? eInp : inp}
                  />
                </Field>
                <Field
                  label={t.fatherName}
                  required
                  error={customerErrors.fatherName}
                >
                  <input
                    value={customer.fatherName}
                    onChange={(e) =>
                      setCustomer((p) => ({ ...p, fatherName: e.target.value }))
                    }
                    className={customerErrors.fatherName ? eInp : inp}
                  />
                </Field>
                ...
                >
                  <input
                    ...
                    className={customerErrors.phoneNumber ? eInp : inp}
                    dir="ltr"
                  />
                </Field>
                <Field
                  label={lang === "dari" ? "شماره تلفن دوم" : "دویم تلیفون"}
                >
                  <input
                    value={customer.altPhone}
                    onChange={(e) =>
                      setCustomer((p) => ({ ...p, altPhone: e.target.value }))
                    }
                    className={inp}
                    dir="ltr"
                  />
                ...
                <Field label={t.occupation}>
                  <input
                    ...
                    onChange={(e) =>
                      setCustomer((p) => ({ ...p, occupation: e.target.value }))
                    }
                    className={inp}
                  />
                </Field>
                <Field label={t.province}>
                  <input
                    ...
                    onChange={(e) =>
                      setCustomer((p) => ({ ...p, province: e.target.value }))
                    }
                    className={inp}
                  />
                </Field>
                <Field label={t.district}>
                  <input
                    ...
                    onChange={(e) =>
                      setCustomer((p) => ({ ...p, district: e.target.value }))
                    }
                    className={inp}
                  />
                </Field>
                <Field label={t.village}>
                  <input
                    ...
                    onChange={(e) =>
                      setCustomer((p) => ({ ...p, village: e.target.value }))
                    }
                    className={inp}
                  />
                </Field>
              ...
              <div className="border-t border-amber-200 pt-4">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  <Field
                    label={t.startDate}
                    required
                    error={customerErrors.startDate}
                  >
                    <AfghanDatePicker
                      value={customer.startDate}
                      onChange={(value) =>
                        setCustomer((p) => ({ ...p, startDate: value }))
                      }
                      defaultToday
                    />
                    <input
                  ...
                  <Field
                    label={t.endDate}
                    required
                    error={customerErrors.endDate}
                  >
                    <AfghanDatePicker
                      value={customer.endDate}
                      onChange={(value) =>
                        setCustomer((p) => ({ ...p, endDate: value }))
                      }
                      defaultToday
                    />
                    <input
                      ...
                      onChange={(e) =>
                        setCustomer((p) => ({ ...p, endTime: e.target.value }))
                      }
                      className={`${inp} mt-1.5`}
                    />
                  </Field>
                ...
                {rentalDays > 0 && (
                  <div className="mt-3 p-3 rounded-xl bg-green-50 border border-green-200 flex items-center gap-3">
                    ...
                    <p className="text-sm text-green-800">
                      ...
                      {lang === "dari" ? "روز انتخاب شده" : "ورځې غوره شوي"}
                      {dailyRent > 0 && (
                        <span>
                          {" "}
                          —{" "}
                          {lang === "dari"
                            ? "کرایه در مرحله مالی تنظیم می‌شود"
                            : "کرایه د مالي مرحلې کې تنظیمیږي"}
                        </span>
                      )}
                    </p>
                  </div>
          ...
          {step === 2 && (
            <div className="space-y-5">
              <StepHeader
                ...
                title={
                  lang === "dari"
                    ? "معلومات ضامن (اختیاری)"
                    : "د ضامن معلومات (اختیاري)"
                }
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                <Field label={t.fullName}>
                  <input
                    ...
                    onChange={(e) =>
                      setGuarantor((p) => ({ ...p, fullName: e.target.value }))
                    }
                    className={inp}
                  />
                </Field>
                ...
                <Field label={t.province}>
                  <input
                    ...
                    onChange={(e) =>
                      setGuarantor((p) => ({ ...p, province: e.target.value }))
                    }
                    className={inp}
                  />
                </Field>
                <Field label={t.district}>
                  <input
                    ...
                    onChange={(e) =>
                      setGuarantor((p) => ({ ...p, district: e.target.value }))
                    }
                    className={inp}
                  />
                </Field>
                ...
                <div className="sm:col-span-2 lg:col-span-3">
                  <Field label={t.notes}>
                    <textarea
                      ...
                      onChange={(e) =>
                        setGuarantor((p) => ({ ...p, notes: e.target.value }))
                      }
                      rows={2}
                      className={`${inp} resize-none`}
                    />
          ...
          {step === 3 && (
            <div className="space-y-5">
              <StepHeader
                ...
                title={
                  lang === "dari"
                    ? "معلومات راننده (اختیاری)"
                    : "د دریور معلومات (اختیاري)"
                }
              ...
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                <Field label={lang === "dari" ? "نام راننده" : "د دریور نوم"}>
                  <input
                    ...
                    className={inp}
                    placeholder={
                      lang === "dari" ? "نام و تخلص..." : "نوم او تخلص..."
                    }
                  />
                </Field>
                <Field label={lang === "dari" ? "نمبر لیسنس" : "لیسنس نمبر"}>
                  <input
                    ...
                    className={inp}
                    placeholder={
                      lang === "dari" ? "شماره لیسنس..." : "لیسنس شمیره..."
                    }
                    dir="ltr"
                  />
                </Field>
                <Field
                  label={
                    lang === "dari" ? "شماره تلفن راننده" : "د دریور تلیفون"
                  }
                >
                  <input
                    value={driverPhone}
          ...
          {step === 4 && (
            <div className="space-y-4">
              <StepHeader
                icon={Receipt}
                title={lang === "dari" ? "محاسبه مالی" : "مالي حساب"}
              ...
              <div className="flex items-center gap-3 px-4 py-3 rounded-2xl border border-amber-200 bg-amber-50/60">
                <div className="w-9 h-9 rounded-xl flex items-center justify-center text-xl bg-amber-100 shrink-0">
                  🚗
                </div>
                <div className="flex-1 min-w-0">
                  <p className="font-bold text-amber-900 truncate text-sm">
                    {selectedCar?.carName}
                    <span className="font-normal text-amber-500 text-xs mr-2">
                      — {selectedCar?.plateNumber}
                    </span>
                  </p>
              ...
              <div className="grid grid-cols-3 gap-3">
                <div className="rounded-2xl p-4 text-center border-2 border-amber-200 bg-amber-50">
                  <p className="text-xs text-amber-600 font-medium mb-1">
                    {lang === "dari" ? "مجموع کرایه" : "ټوله کرایه"}
                  </p>
                ...
                <div className="rounded-2xl p-4 text-center border-2 border-green-300 bg-green-50">
                  <p className="text-xs text-green-600 font-medium mb-1">
                    {lang === "dari" ? "دریافت شده" : "ترلاسه شوي"}
                  </p>
                ...
                <div
                  className={`rounded-2xl p-4 text-center border-2 ${remaining > 0 ? "border-red-300 bg-red-50" : "border-emerald-300 bg-emerald-50"}`}
                >
                  <p
                    className={`text-xs font-medium mb-1 ${remaining > 0 ? "text-red-600" : "text-emerald-600"}`}
                  >
                    {lang === "dari" ? "باقی‌مانده" : "پاتې"}
                  </p>
                  <p
                    className={`text-xl font-black ${remaining > 0 ? "text-red-700" : "text-emerald-700"}`}
                  >
                    {formatNumber(remaining)}
                  </p>
                  <p
                    className={`text-[10px] mt-0.5 ${remaining > 0 ? "text-red-400" : "text-emerald-400"}`}
                  >
                    افغانی
                  </p>
                </div>
          ...
          {step === 5 && (
            <div className="space-y-6">
              <StepHeader
                icon={FileImage}
                title={lang === "dari" ? "آپلود اسناد" : "د اسنادو پورته کول"}
                gradient="linear-gradient(135deg,#0891b2,#0e7490)"
              />
              ...
              <div className="space-y-5">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
                  <DocSlot
                    ...
                    sublabel={
                      lang === "dari" ? "برای تأیید هویت" : "د هویت تایید لپاره"
                    }
                    preview={customerPhotoPreview}
                    onFile={(f) => {
                      setCustomerPhoto(f);
                      setCustomerPhotoPreview(URL.createObjectURL(f));
                    }}
                    onClear={() => {
                  ...
                  <DocSlot
                    ...
                    onFile={(f) => {
                      setBillDocPhoto(f);
                      setBillDocPhotoPreview(URL.createObjectURL(f));
                    }}
                    onClear={() => {
                      setBillDocPhoto(null);
                      setBillDocPhotoPreview(null);
                    }}
                    color="#059669"
                  />
                </div>
                ...
                <MultiDocSlot
                  ...
                  sublabel={
                    lang === "dari"
                      ? "حداکثر ۲ تصویر را با هم انتخاب کنید"
                      : "تر ۲ پورې انځورونه یو ځل وټاکئ"
                  }
                ...
                {remaining > 0 && (
                  <>
                    ...
                    <span className="font-semibold text-red-600">
                      {lang === "dari" ? "باقی:" : "پاتې:"} {fmtCur(remaining)}
                    </span>
                  </>
                )}
              </div>
            ...
            {step < 5 ? (
              ...
              >
                {saving ? (
                  ...
                  <>
                    <Check className="w-4 h-4" />
                    {lang === "dari" ? "ذخیره تغییرات" : "بدلونونه خوندي کول"}
                  </>
                ) : (
                  <>
                    <Check className="w-4 h-4" />
