...
export default function DashboardPage() {
  ...
  const totalMoney    = contracts.reduce((s, c) => s + (c.totalRent || 0), 0);
  const totalReceived = contracts.reduce((s, c) => s + ((c.totalRent || 0) - (c.remainingAmount || 0)), 0);

  /* derived payment data */
  const allPayments = contracts.flatMap(c =>
    (c.payments || []).map((p: any) => ({ ...p, contract: c }))
  ).sort((a: any, b: any) => new Date(b.createdAt).getTime() - new Date(a.createdAt).getTime());

  const pendingContracts = contracts.filter(c => c.remainingAmount > 0 && c.status === 'ACTIVE');
  const totalCollected   = allPayments.reduce((s: number, p: any) => s + p.amount, 0);
  const totalPending     = pendingContracts.reduce((s: number, c: any) => s + c.remainingAmount, 0);
  ...
  if (isLoading) return (
    <MainLayout>
      <div className="flex items-center justify-center h-64">
        <div className="text-amber-600 text-lg animate-pulse">{t.loading}</div>
      </div>
    </MainLayout>
  );
    ...
    <MainLayout>
      <div className="space-y-6">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          {/* Payment tabs panel — 2/3 width */}
          <div className="lg:col-span-2 card-golden rounded-2xl overflow-hidden">
            <div className="px-5 pt-4 pb-0 border-b border-amber-200 flex items-center justify-between gap-4">
              <div className="flex gap-1">
                {([
                  ...
                  <button key={tab.key} onClick={() => setPayTab(tab.key)}
                    className={`px-4 py-2.5 text-sm font-medium border-b-2 -mb-px transition-all ${payTab === tab.key ? 'border-amber-500 text-amber-800' : 'border-transparent text-amber-500 hover:text-amber-700'}`}>
                    {tab.label}
                    {tab.key === 'pending' && pendingContracts.length > 0 &&
                      <span className="mr-1.5 text-xs px-1.5 py-0.5 rounded-full bg-red-100 text-red-600">{pendingContracts.length}</span>
                    }
                  </button>
                ))}
              </div>
              <Link href="/contracts" className="text-amber-500 text-xs hover:text-amber-700 flex items-center gap-1 pb-2.5">
            ...
            <div className="p-4">
              ...
              {payTab === 'pending' && (
                pendingContracts.length === 0
                  ? <div className="text-center py-10 text-amber-400 flex flex-col items-center gap-2">
              ...
              {payTab === 'latest' && (
                allPayments.length === 0
                  ? <div className="text-center py-10 text-amber-400 text-sm">{t.noData}</div>
                  : <div className="overflow-x-auto">
                      <table className="w-full table-golden min-w-[420px]">
                        <thead><tr>
                          <th className="px-3 py-2 text-right text-xs">{t.contractNo}</th>
                          <th className="px-3 py-2 text-right text-xs">{t.customer}</th>
                          <th className="px-3 py-2 text-right text-xs">{t.amount}</th>
                          <th className="px-3 py-2 text-right text-xs">{t.date}</th>
                        </tr></thead>
                        <tbody>
                          {allPayments.slice(0, 10).map((p: any) => (
                            <tr key={p.id} className="border-b border-amber-100">
                              <td className="px-3 py-2 text-xs font-mono text-amber-600">{p.contract?.contractNumber}</td>
                              <td className="px-3 py-2 text-xs">{p.contract?.customer?.fullName}</td>
                  ...
                  return (
                    <div key={month}>
                      <div className="flex justify-between items-center mb-1">
                        ...
                        <span className="text-xs font-medium text-amber-900">{formatCurrency(amount)}</span>
                      </div>
        ...
        <div className="card-golden rounded-2xl overflow-hidden">
          <div className="p-5 border-b border-amber-200 flex justify-between items-center">
            ...
            <table className="w-full table-golden">
              ...
              <tbody>
                {stats?.recentContracts?.map((c: any) => (
                  <tr key={c.id} className="border-b border-amber-100 hover:bg-amber-50/40 transition-colors">
                    ...
                    <td className="px-4 py-3 text-sm font-medium text-amber-900">{formatCurrency(c.totalRent)}</td>
                    <td className="px-4 py-3 text-sm font-medium text-red-600">{formatCurrency(c.remainingAmount)}</td>
                    <td className="px-4 py-3">
                      <span className={`inline-flex px-2 py-0.5 rounded-full text-xs font-medium ${statusLabel[c.status]?.cls}`}>
