...
const statusMap: Record<
  string,
  { dari: string; pashto: string; dot: string; bg: string; text: string }
> = {
  ACTIVE: {
    dari: "فعال",
    pashto: "فعال",
...
export default function DashboardPage() {
  ...
  const availablePct = totalCars
    ? Math.round(((stats?.availableCars || 0) / totalCars) * 100)
    : 0;
  const rentedPct = totalCars
    ? Math.round(((stats?.rentedCars || 0) / totalCars) * 100)
    : 0;
  const maintenancePct = totalCars
    ? Math.round(100 - availablePct - rentedPct)
    : 0;
      ...
      <MainLayout>
        <div className="space-y-6 page-enter">
          ...
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
            {[...Array(4)].map((_, i) => (
              <div key={i} className="card-golden rounded-2xl p-5 space-y-3">
                <Sk h="h-10" />
                <Sk w="w-3/4" h="h-5" />
                <Sk w="w-1/2" h="h-4" />
              </div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
            {[...Array(3)].map((_, i) => (
              <div
                key={i}
                className="card-golden rounded-2xl p-5 space-y-3 h-40"
              >
                <Sk />
                <Sk w="w-3/4" />
              </div>
      ...
      <div className="space-y-6 page-enter">
        ...
        <div className="flex flex-wrap items-center justify-between gap-4">
          <div className="flex items-center gap-3">
            <div
              className="w-11 h-11 rounded-2xl overflow-hidden flex items-center justify-center shrink-0 shadow-md"
              style={{
                background: "#fff",
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-5 gap-4">
          ...
          <div className="card-golden rounded-2xl overflow-hidden">
            ...
            <div className="p-5">
              ...
              <p
                className={`text-2xl font-black leading-tight ${(stats?.adminNetIncome ?? 0) >= 0 ? "text-emerald-800" : "text-red-700"}`}
              >
              ...
              <p className="text-xs text-gray-400 mt-0.5">
                {d("بعد از کسر مصارف", "د لګښتونو وروسته")}
              </p>
            </div>
          </div>
          ...
          <div className="card-golden rounded-2xl overflow-hidden">
            ...
            <div className="p-5">
              ...
              <p className="text-xs text-red-400 mt-0.5">
                {d("سهم ادمین از مصارف", "د ادمین د لګښتونو برخه")}
              </p>
            </div>
          </div>
            ...
            <div className="card-golden rounded-2xl overflow-hidden">
              ...
              <div className="p-5">
                ...
                <p className="text-xs text-orange-400 mt-0.5">
                  {d("شامل در مجموع کرایه", "په ټوله کرایه کې شامله")}
                </p>
              </div>
            </div>
          )}
          ...
          {[
            ...
            },
            {
              ...
              value: fc(
                (stats?.totalContractValue ?? 0) +
                  (stats?.totalDelayPenalty ?? 0),
              ),
              sub: d(
                "مجموع ارزش سفارش‌ها + جریمه",
                "د ټولو سفارشونو ارزښت + جریمه",
              ),
              icon: Receipt,
            ...
            >
              ...
              <div className="min-w-0 flex-1">
                <p className="text-xs text-gray-500 font-medium mb-0.5 leading-tight">
                  {label}
                </p>
                <p className="text-lg font-black text-amber-900 truncate leading-tight">
                  {value}
                </p>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
          ...
          <div className="card-golden rounded-2xl p-5 space-y-4">
            <SectionHead
              title={d("وضعیت موترها", "د موترونو حالت")}
              icon={Car}
              color="linear-gradient(135deg,#f59e0b,#d97706)"
              link="/cars"
              linkLabel={d("موترها", "موترونه")}
            />
                ...
                <svg viewBox="0 0 36 36" className="w-24 h-24 -rotate-90">
                  ...
                  {availablePct > 0 && (
                    <circle
                      ...
                      strokeDashoffset={`${-(rentedPct + maintenancePct)}`}
                      strokeLinecap="round"
                    />
                  )}
                </svg>
          ...
          <div className="card-golden rounded-2xl p-5 space-y-4">
            <SectionHead
              title={d("وضعیت سفارش‌ها", "د سفارشونو حالت")}
              icon={FileText}
            ...
            <div className="space-y-3">
              {[
                ...
                {
                  label: d("ناوقت", "ناوخته"),
                  value: stats?.overdueContracts || 0,
                  color: "#ef4444",
                  bg: "#fee2e2",
                  icon: AlertCircle,
                },
                {
                  label: d("مشتریان", "مشتریان"),
          ...
          <div className="card-golden rounded-2xl p-5 space-y-4">
            ...
            />
            {stats?.monthlyIncome &&
            Object.keys(stats.monthlyIncome).length > 0 ? (
              <div className="space-y-3">
                {Object.entries(stats.monthlyIncome)
                  .slice(-5)
                  .reverse()
                  .map(([month, amount]: any) => {
                    const max = Math.max(
                      ...(Object.values(stats.monthlyIncome) as number[]),
                    );
            ...
            <div className="flex">
              {(
                [
                  {
                    key: "pending",
                    label: d("پرداخت‌های باقی‌مانده", "پاتې تادیات"),
                    count: pendingList.length,
                  },
                  {
                    key: "latest",
                    label: d("آخرین تراکنش‌ها", "وروستي لیږدونه"),
                    count: recentPays.length,
                  },
                ] as const
              ).map((tab) => (
                <button
              ...
              (pendingList.length === 0 ? (
                <div className="flex flex-col items-center justify-center py-12 gap-3">
                  ...
                  <p className="text-sm font-semibold text-gray-500">
                    {d(
                      "همه پرداخت‌ها تسویه شده است",
                      "ټولې تادیې تسویه شوي دي",
                    )}
                  </p>
                </div>
              ) : (
                  ...
                  <table className="w-full min-w-[560px]">
                    ...
                    <tbody className="divide-y divide-amber-50">
                      {pendingList.map((c: any, i: number) => {
                        ...
                        const overdueCharge =
                          c.liveOverdueCharges ?? c.liveTotalDelayPenalty ?? 0;
                        const finalTotal = c.liveFinalTotal ?? c.totalRent;
                        return (
                          <tr
                            key={c.id}
                            className={`hover:bg-amber-50/50 transition-colors ${isOverdue ? "bg-red-50/30" : ""}`}
                          >
                ...
                <tbody className="divide-y divide-amber-50">
                  {stats.recentContracts.map((c: any, i: number) => {
                    ...
                    return (
                      <tr
                        key={c.id}
                        className={`hover:bg-amber-50/40 transition-colors ${isOverdue ? "bg-red-50/20" : ""}`}
                      >
                        ...
                        <td className="px-4 py-3.5">
                          <div className="flex flex-col gap-0.5">
                            ...
                            {isOverdue &&
                              (c.liveOverdueCharges ??
                                c.liveTotalDelayPenalty ??
                                0) > 0 && (
                        ...
                        <td className="px-4 py-3.5">
                          <span
                            className={`text-sm font-bold ${c.remainingAmount > 0 ? "text-red-600" : "text-emerald-600"}`}
                          >
                            {fc(c.remainingAmount)}
                          </span>
                        </td>
