...
import {
  ...
  calcLiveOverdue,
  buildDeadline,
} from "../utils/overdueUtils.js";
import { parseKabulDateString, formatKabulIso } from "../utils/dateUtils.js";
...
const fmtDate = (d) => formatKabulIso(d);
...
export const createContract = async (req, res) => {
  try {
    ...
    if (rentPrice < 0)
      return sendError(res, "کرایه روزانه نمی‌تواند منفی باشد", 400);
    if (totalRent < 0)
      return sendError(res, "مجموع کرایه نمی‌تواند منفی باشد", 400);
    if (advance < 0)
      return sendError(res, "پیش پرداخت نمی‌تواند منفی باشد", 400);
    if (!isNaN(remaining) && remaining < 0)
      return sendError(res, "باقی مانده نمی‌تواند منفی باشد", 400);
    if (delayPenaltyRate < 0)
      return sendError(res, "نرخ جریمه نمی‌تواند منفی باشد", 400);
    ...
    const data = {
      ...
      startDate: parseKabulDateString(req.body.startDate),
      startTime: req.body.startTime || "00:00",
      endDate: parseKabulDateString(req.body.endDate),
      endTime: req.body.endTime || "00:00",
      rentPrice,
      totalRent,
      advancePayment: advance,
      remainingAmount: isNaN(remaining)
        ? Math.max(0, totalRent - advance)
        : remaining,
      carStatus: req.body.carStatus || null,
      agreementConfirmed:
        req.body.agreementConfirmed === true ||
        req.body.agreementConfirmed === "true",
...
export const updateContract = async (req, res) => {
  try {
    ...
    if (current.status === "COMPLETED" && current.completedAt) {
      const elapsedHours =
        (Date.now() - new Date(current.completedAt).getTime()) / 3_600_000;
      if (elapsedHours > 24) {
        return sendError(
          res,
    ...
    [
      ...
      if (updateData[f] !== undefined)
        updateData[f] = parseFloat(updateData[f]) || 0;
    });
    if (updateData.agreementConfirmed !== undefined) {
      updateData.agreementConfirmed =
        updateData.agreementConfirmed === true ||
        updateData.agreementConfirmed === "true";
    }
    if (updateData.startDate)
      updateData.startDate = parseKabulDateString(updateData.startDate);
    if (updateData.endDate)
      updateData.endDate = parseKabulDateString(updateData.endDate);
    ...
    if (updateData.carId && updateData.carId !== existing.carId) {
      await prisma.car
        ...
        .catch(() => {});
      await prisma.car
        .update({ where: { id: updateData.carId }, data: { status: "RENTED" } })
        .catch(() => {});
    }
    ...
    if (
      ...
      updateData.totalRent === undefined
    ) {
...
export const markAsReturned = async (req, res) => {
  try {
    ...
    const penaltyRate =
      existing?.delayPenaltyRate > 0
        ? existing.delayPenaltyRate
        : existing?.rentPrice || 0;
...
export const deleteContract = async (req, res) => {
  try {
    ...
    await prisma.car
      .update({ where: { id: contract.carId }, data: { status: "AVAILABLE" } })
      .catch(() => {});
    ...
    if (remainingContracts === 0) {
      ...
      await prisma.customer
        .delete({ where: { id: contract.customerId } })
        .catch(() => {});
